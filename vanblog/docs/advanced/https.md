...
BianBlog 镜像内采用了 Caddy 作为反向代理，并支持全自动按需 HTTPS 证书申请配置。
...
BianBlog 首次运行默认关闭 HTTPS，请通过 HTTP 协议访问。无需多余设置，首次通过 “HTTPS + 域名” 访问时，会自动申请 HTTPS 证书并应用。
...
BianBlog 通过 Caddy 的 API 在运行时动态修改配置来开关 HTTPS 自动重定向。
...
- BianBlog 访问日志在容器中的 `/var/log/bianblog-access.log`
