...
如果你想参与 BianBlog 开发，可以进群哦：

- [BianBlog 开发群](https://jq.qq.com/?_wv=1027&k=mf2CguM8)

## 准备知识

### 整体架构

BianBlog 分为以下几个部分，构建后将整合到一个 `docker` 容器内：

> website: BianBlog 默认的主题，使用了 `nextjs` 框架，有运行时。
>
> server: BianBlog 的后端服务，有运行时。
>
> waline: BianBlog 内嵌的评论服务，有运行时。
>
> admin: BianBlog 后台面板，打包后为静态页面，无运行时。
>
...
```bash
docker run --name mongodb-bianblog -d --restart unless-stopped \
  -p 27017:27017 mongo
```
...
```bash
git clone https://github.com/Mereithhh/bianblog.git
cd bianblog
pnpm i
```
...
database:
  # 数据库连接
  url: mongodb://localhost:27017/bianBlog?authSource=admin
static:
  # 图床等静态文件保存的位置
  path: /var/bianblog-dev/static
# 是否开启演示站模式，会限制很多权限
demo: 'false'
# waline 用的表名，会自动创建
waline:
...
::: info BianBlog开发后台如果用到复制到剪切板相关的功能，可能需要开启 `https`，请在 `packages/admin/config/config.js` 中的 `https` 改成 `true`，再重启开发进程。
...
BIAN_BLOG_BUILD_SERVER="https://some.bianblog-server.com"
docker build --build-arg BIAN_BLOG_BUILD_SERVER=$BIAN_BLOG_BUILD_SERVER -t mereith/bian-blog:test .
```
