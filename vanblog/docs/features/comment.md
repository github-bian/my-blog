...
BianBlog 内嵌了 [Waline 评论系统](https://waline.js.org/)，你不需要任何额外的配置或额外部署，开箱即用。
...
如果还是不生效，可以尝试一下重启 BianBlog，可能会解决问题。

:::

### 配置 webhook 消息通知

BianBlog 内嵌的评论系统支持在有新评论时发送 `webhook`，配置好 `webhook` 接收地址后，会发送一条 `POST` 请求，具体包含以下请求体（JSON 格式）：
