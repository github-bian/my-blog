...
BianBlog 带有内置的图床实现，无需任何配置，开箱即用。
...
BianBlog 可以对接第三方图床，是基于 [picgo-core](https://picgo.github.io/PicGo-Core-Doc/) 实现的。
...
- [bianblog 使用 picgo 图床的完整部署教程](https://www.handyzyg.cn/post/47)
...
BianBlog 使用 `picgo-core` 配置文件，和桌面版的 `picgo` 的配置文件不通用！
...
无论使用何种图床，BianBlog 都支持上传图片自动压缩为 webp 格式。在编辑器或图片管理中上传图片时，BianBlog 会在收到图片信息的时候，先进行压缩，再保存到对应图床。
...
无论使用何种图床，BianBlog 都支持上传图片自动添加水印。在编辑器或图片管理中上传图片时，BianBlog 会在收到图片信息的时候，先进行水印转换，再保存到对应图床。
...
BianBlog 支持把文章中存在的，但是不在图床记录中的图片扫描出来。
...
进入 BianBlog 后台的 `图片管理` 模块可以对图片进行管理：
