...
BianBlog 现在支持一键脚本部署了。经过测试，宝塔也可以通过一键脚本进行部署。
...
你也可以通过宝塔面板图形化操作部署 BianBlog，具体步骤如下：
...
如图所示，添加 `docker-compose` 模板，模板名称为 `bianblog`，描述随意。
...
services:
  bianblog:
    # 阿里云镜像源
    # image: registry.cn-beijing.aliyuncs.com/mereith/bian-blog:latest
    image: mereith/bian-blog:latest
    restart: always
    environment:
      TZ: 'Asia/Shanghai'
    ...
    volumes:
      # 图床文件的存放地址，按需修改。
      - /var/bianblog/data/static:/app/static
      # 日志文件
      - /var/bianblog/log:/var/log
      # Caddy 配置存储
      - /var/bianblog/caddy/config:/root/.config/caddy
      # Caddy 证书存储
      - /var/bianblog/caddy/data:/root/.local/share/caddy
    ports:
  ...
  mongo:
    ...
    environment:
      TZ: 'Asia/Shanghai'
    volumes:
      - /var/bianblog/data/mongo:/data/db
```
...
如下图所示，新建 `Compose` 项目，名称写 `bianblog`，模板选择刚刚创建的。
...
如果你只部署 BianBlog ，并想关闭 Ngnix ，请输入以下命令关闭 Ngnix:
