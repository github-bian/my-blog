...
和上面一样，点击 `容器/新建` ，选中刚刚下载的 `mereith/bian-blog:latest` 镜像，容器名称为 BianBlog。
...
为了让 BianBlog 容器内可以访问之前创建的 `mongo` 容器，在 `高级设置/链接` 中添加链接到之前创建的 `mongo` 容器。
...
可以参考 [BianBlog 文档/环境变量配置](../reference//env.md) 中的目录映射部分进行映射。
