...
目前 BianBlog 处于快速迭代期，如果后台出现新版本提醒，推荐进行升级。
...
你可以直接运行安装脚本来升级 BianBlog，启动后请输入 6 并回车。
...
请切换到部署 BianBlog 的目录下（docker-compose.yaml 存放的路径下），然后运行下面的命令。

```bash
# 关闭原有服务
docker-compose down -v
# 删除原有镜像
docker rmi mereith/bian-blog:latest
# 重新拉取最新镜像
docker pull mereith/bian-blog:latest
# 重新启动服务
docker-compose up -d
```
...
`mereith/bian-blog:latest` 为 `mereith/bian-blog:v0.29.0` ，然后运行：
