...
import { BianBlogSystemEvent, BianBlogSystemEventNames } from 'src/types/event';
...
export class PipelineProvider {
  ...
  checkEvent(eventName: string) {
    if (BianBlogSystemEventNames.includes(eventName)) {
      return true;
    }
    return false;
  }
  ...
  async createPipeline(pipeline: CreatePipelineDto) {
    if (!this.checkEvent(pipeline.eventName)) {
      throw new NotFoundException('Event not found in BianBlogEventNames');
    }
...
`;
  ...
  async dispatchEvent(eventName: BianBlogSystemEvent, data?: any) {
