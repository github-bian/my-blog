...
export class WebsiteProvider {
  ...
  async loadEnv() {
    ...
    const isrEnv =
      isrConfig.mode == 'delay'
        ? {
            BIAN_BLOG_REVALIDATE: 'true',
            BIAN_BLOG_REVALIDATE_TIME: isrConfig.delay,
          }
        : {
            BIAN_BLOG_REVALIDATE: 'false',
          };
    ...
    return { BIAN_BLOG_ALLOW_DOMAINS: urls.join(','), ...isrEnv };
  }
  ...
  async run(): Promise<any> {
    if (process.env['BIANBLOG_DISABLE_WEBSITE'] === 'true') {
