export interface BianBlogEventItem {
  eventName: string;
...
export type BianBlogEventType = 'system' | 'custom' | 'corn';

export const BianBlogSystemEvents: BianBlogEventItem[] = [
  {
    eventName: 'login',
    eventNameChinese: '登录',
    eventDescription: '登录',
...
export const BianBlogSystemEventNames = BianBlogSystemEvents.map((item) => item.eventName);
export type BianBlogSystemEvent =
  | 'login'
  | 'logout'
  | 'beforeUpdateArticle'
  | 'afterUpdateArticle'
