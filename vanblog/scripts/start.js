...
const logPathEnv = process.env.BIAN_BLOG_LOG;
if (logPathEnv) {
  logPath = logPathEnv;
}

const printLog = (string, isError = false) => {
  const logName = `bianblog-${isError ? 'stderr' : 'stdout'}.log`;
  const logNameNormal = `bianblog-stdio.log`;
...
ctx.on('exit', () => {
  process.stderr.write(`[bianblog] 已停止`);
});
